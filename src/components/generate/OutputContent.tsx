      ...
      <Tabs defaultValue='output' value={currentTab} className='w-full h-[460px]'>
        <div className='flex justify-center mb-6'>
          <TabsList className='rounded-full p-1 bg-transparent border border-[#272626]'>
            <TabsTrigger onClick={() => setCurrentTab('output')} className='rounded-full text-white' value='output'>
              Output
            </TabsTrigger>
            <TabsTrigger onClick={() => setCurrentTab('history')} className='rounded-full text-white' value='history'>
              Contents
            </TabsTrigger>
          </TabsList>
        </div>

        <TabsContent value='output' className='h-full bg-[#9f9f9f]/5 rounded-lg'>
          <div className='h-full md:h-[455px] rounded-lg border border-black/5 px-5 py-4 overflow-auto'>
            {content ? (
              ...
              <p className='text-sm text-white'>See the output here...</p>
            )}
          </div>
        </TabsContent>
